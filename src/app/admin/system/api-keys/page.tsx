    ...
    return (
      <div className="min-h-[50vh] flex items-center justify-center text-center">
        <div className="space-y-3">
          ...
          <p className="text-xs font-bold text-slate-400">در حال دریافت کلیدهای کل سیستم...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-fade-in text-right">
      <div className="space-y-2">
        <h1 className="text-2xl sm:text-3xl font-black dark:text-white text-slate-900 flex items-center gap-3">
          ...
          تنظیمات متمرکز & صدور کلیدهای API ارشد ادمین (Admin Security)
        </h1>
        <p className="text-xs sm:text-sm dark:text-slate-400 text-slate-600">
          مدیریت دسترسی‌های سطح بالای `X-ANPK-API-KEY` جهت فراخوانی متدهای ادمین جنگو.
        </p>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl glass-card border dark:border-slate-800 border-slate-200 space-y-6 shadow-xl">
        <div className="flex items-center gap-2 border-b dark:border-slate-800 border-slate-200 pb-4">
          ...
          <h2 className="text-lg font-black dark:text-white text-slate-900">صدور کلید دسترسی ارشد ادمین</h2>
        </div>
        ...
        <form onSubmit={handleCreateKey} className="flex flex-col sm:flex-row items-center gap-4">
          <input
            ...
            placeholder="عنوان کلید ادمین (مثلاً: کلید اتصال سرویس مانیتورینگ نودها)"
          ...
          >
            ...
            <span>{submitting ? 'در حال صدور...' : 'صدور کلید ادمین'}</span>
          </button>
        </form>
      </div>
