...
export default function AdminPortfolioPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [pRes, metaRes] = await Promise.all([
        adminFetch(`${API_BASE}/api/portal/admin/portfolio-projects/`),
        adminFetch(`${API_BASE}/api/portal/admin/project-metadata/`),
      ]);
      setPortfolio(pRes || []);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/api/portal/admin/project-metadata/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res?.message) {
        ...
        setNewCatName('');
        // Reload categories
        const metaRes = await adminFetch(`${API_BASE}/api/portal/admin/project-metadata/`);
        setCategories(metaRes?.categories || []);
      } else if (res?.error) {
  ...
  const handleDeleteCategory = async (id: number) => {
    ...
    try {
      await adminFetch(`${API_BASE}/api/portal/admin/project-metadata/`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
