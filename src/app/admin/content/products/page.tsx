...
export default function AdminProductsPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [pRes, cRes] = await Promise.all([
        adminFetch(`${API_BASE}/api/v1/admin/products/`),
        adminFetch(`${API_BASE}/api/v1/admin/product-categories/`),
      ]);
      setProducts(pRes || []);
      setCategories(cRes || []);
    } catch (err) {
  ...
  const openEditModal = async (p: any) => {
    ...
    try {
      const features = await adminFetch(`${API_BASE}/api/v1/admin/product-features/?product_id=${p.id}`);
  ...
  const handleSubmitProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await adminFetch(`${API_BASE}/api/v1/admin/products/`, {
  ...
  const handleDeleteProduct = async (id: number) => {
    ...
    try {
      await adminFetch(`${API_BASE}/api/v1/admin/products/?id=${id}`, { method: 'DELETE' });
  ...
  const handleAddFeature = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await adminFetch(`${API_BASE}/api/v1/admin/product-features/`, {
        method: 'POST',
      ...
      if (res?.message) {
        showMsg('success', res.message);
        setNewFeature({ title: '', description: '', icon_name: 'CheckCircle2' });
        const features = await adminFetch(`${API_BASE}/api/v1/admin/product-features/?product_id=${editProdId}`);
        setProductFeatures(features || []);
      }
  ...
  const handleDeleteFeature = async (featId: number) => {
    ...
    try {
      await adminFetch(`${API_BASE}/api/v1/admin/product-features/?id=${featId}`, { method: 'DELETE' });
  ...
  const handleSubmitCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await adminFetch(`${API_BASE}/api/v1/admin/product-categories/`, {
        method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body)
      });
      if (res?.message) {
        showMsg('success', res.message);
  ...
  const handleDeleteCategory = async (id: number) => {
    ...
    try {
      await adminFetch(`${API_BASE}/api/v1/admin/product-categories/?id=${id}`, { method: 'DELETE' });
