...
export default function AdminCommentsPage() {
  ...
  const fetchComments = async () => {
    ...
    try {
      const url = statusFilter 
        ? `${API_BASE}/api/v1/admin/comments/?status=${statusFilter}`
        : `${API_BASE}/api/v1/admin/comments/`;
  ...
  const updateStatus = async (id: number, newStatus: string) => {
    try {
      const res = await adminFetch(`${API_BASE}/api/v1/admin/comments/`, {
        method: 'PUT',
  ...
  const deleteComment = async (id: number) => {
    ...
    try {
      const res = await adminFetch(`${API_BASE}/api/v1/admin/comments/?id=${id}`, {
        method: 'DELETE'
      });
      if (res?.message) {
        showMsg('success', res.message);
